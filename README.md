...
An Arc-inspired recent-tab switcher for Chrome. Hold **Control**, press **Q** repeatedly to browse tabs in the order you last visited them, then release Control to switch. Local screenshot previews are optional.

**[Install with an agent](./INSTALL_WITH_AGENT.md)** · [中文说明](#中文说明) · [Verification](./VERIFY.md)

![Recent Tabs preview cards](./assets/preview.png)

Current development version: **0.1.3**. Requires **Chrome 127+**. This is an independent project inspired by Arc; it is not affiliated with Arc or Chrome.

## Install

The extension is distributed as an unpacked extension. **No build, Node.js, Python, or npm installation is needed.**

1. Download or clone this repository's current `main` branch, then keep the complete folder in a permanent local location. The published `recent-tabs-portable-v0.1.2.zip` predates the Control-release fixes; there is no 0.1.3 release ZIP yet.
...
6. Optional: open settings using the gear icon and click **Enable website previews** to grant website access for local screenshots. Visit a few ordinary pages and pause briefly to populate that cache.
...
Website access is optional. On ordinary HTTP(S) pages, the keyboard shortcut opens in-page title cards and commits on Control release whether or not screenshots are enabled. Restricted pages use the toolbar popup. After installation, **do not move or delete the loaded folder**. Unpacked extensions do not automatically install on another computer through Chrome Sync; load the folder and grant permissions separately on each computer.
...
The current `main` branch (0.1.3) opens in-page title cards for normal web pages even when website previews are off. It keeps page focus until the card UI can receive the keyboard, and releasing Control activates the final selection after queued shortcuts. Reload the unpacked extension in `chrome://extensions` and refresh existing web pages after updating, then confirm **Version 0.1.3** in its settings. The published v0.1.2 ZIP predates these fixes.
...
- Optional website access enables automatic screenshot capture of the page you are viewing. Disabling previews removes this permission; it does not disable in-page title cards or Control-release switching on ordinary web pages.
...
The toolbar popup on a restricted page may miss a very fast Control release while focus moves from browser UI to the panel. On an ordinary page, the first gesture can also miss an exceptionally fast release if it happens before the temporary page listener finishes injecting. If a panel remains open, press **Enter** to confirm or **Esc** to cancel, then retry the gesture.
...
The current test suite includes regression coverage for startup focus, early Control release, queued shortcuts, and cancellation. The **37 logic tests** and **10 Chrome browser scenarios** recorded for the v0.1.2 release are historical; browser scenarios have not been rerun for 0.1.3. See [VERIFY.md](./VERIFY.md) for current validation and reproduction commands.
...
1. 下载或克隆本仓库当前的 `main` 分支，把完整文件夹放到准备长期保留的位置。已发布的 `recent-tabs-portable-v0.1.2.zip` 早于 Control 松键修复；目前还没有 0.1.3 的 release ZIP。无需构建，也不需要安装 Node.js、Python 或 npm。
...
4. 可选：点击扩展图标中的齿轮 → **Enable website previews**，允许网站访问权限以保存本地预览截图。依次浏览几个普通网页并稍作停留，即可积累预览截图。
...
当前 `main`（0.1.3）在普通 HTTP(S) 网页上即使未启用网站预览，也会打开页面内标题卡片。面板准备好才接收焦点，松开 Control 后切换到连续按键最终选中的标签。更新后在 `chrome://extensions` 重新加载扩展、刷新原来已打开的网页，并在设置页确认显示 **Version 0.1.3**；已发布的 v0.1.2 ZIP 尚不包含这些修复。

按 A → B → C → D 的顺序访问四个标签后，在 D 按住 Control 连按 Q，会依次选中 C、B、A；加 Shift 反向，Esc 取消，Enter 或点击卡片也能确认。预览是上次浏览时保存在本机会话中的截图，不是后台实时画面；可以在设置中清空。关闭预览不会关闭普通网页上的页面内标题卡片和松键切换。Chrome 内置页等受限页面会使用工具栏备用弹窗。

**完整的物理按住／松键手势仍待人手验收**，松键逻辑已通过浏览器输入测试，原生快捷键分发、选择顺序、预览及 Enter／Esc 已分别验证。受限页的工具栏弹窗从地址栏发起时可能丢失极快松键；普通网页首次调用时，若在临时页面监听器注入完成前极快松键，也可能丢失。若面板未关闭，用 Enter 确认或 Esc 取消，再重试手势。Windows、Linux 和可选的 [Control + Tab 系统映射](./macos/README.md) 尚未实测。完整验证范围见 [VERIFY.md](./VERIFY.md)。
