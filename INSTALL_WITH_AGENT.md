...
Choose **one** of the equivalent prompts below and paste it into your agent. Both prompts request installation, shortcut setup, optional screenshot-preview permission, and verification.

选择下面任意一种语言的指令交给 agent 即可；两版都包含安装、快捷键配置、可选截图预览授权和验证。
...
First read AGENTS.md and README.md in the project root and follow their installation instructions. Read the actual version from extension/manifest.json; the current main branch is expected to be 0.1.3. The published 0.1.2 ZIP predates the Control-release fixes, so do not treat it as this version.
...
I authorize you to pin the extension icon and enable website previews in its settings, granting the required website-access permission through Chrome's native permission prompt. Preview screenshots must remain in this device's extension session storage. First verify that normal web pages switch through in-page title cards with previews disabled; preview permission is only for screenshots. Do not copy the old computer's browsing data or credentials. Control + Q does not require Karabiner; do not install it.
...
When finished, report the actual version shown in settings, permanent installation path, Chrome profile, both saved shortcuts and their scopes, whether previews are enabled, the checks that passed, and any remaining blockers. Include a local link to README.md.
```
...
先读取项目根目录的 AGENTS.md 和 README.md，严格按安装指令执行。以 extension/manifest.json 的实际版本为准；当前 main 分支目标为 0.1.3。已发布的 0.1.2 ZIP 早于 Control 松键修复，不能把它当作这个版本。
...
我授权你固定扩展图标，并在扩展设置中启用 website previews，通过 Chrome 的原生权限界面授予所需的网站访问权限。预览截图应保留在本机扩展会话中。请先在未启用预览时验证普通网页仍会打开页面内标题卡片并完成切换；预览权限只用于截图。不复制旧电脑的浏览数据或凭据。Control + Q 不需要 Karabiner，不要安装它。
...
完成后告诉我设置页显示的实际版本、永久安装路径、Chrome 配置文件、两项实际快捷键和范围、预览是否启用、哪些验收已通过以及剩余阻碍，并给出 README.md 的本机链接。
```
